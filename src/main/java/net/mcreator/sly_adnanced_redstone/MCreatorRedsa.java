...
public class MCreatorRedsa extends Elementssly_adnanced_redstone.ModElement {
	@ObjectHolder("sly_adnanced_redstone:redsa")
	public static final ModDimension dimension = null;
	@ObjectHolder("sly_adnanced_redstone:redsa")
	public static final Item block = null;
	@ObjectHolder("sly_adnanced_redstone:redsa_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;

	public MCreatorRedsa(Elementssly_adnanced_redstone instance) {
		super(instance, 38);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("redsa"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("sly_adnanced_redstone:redsa")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("sly_adnanced_redstone:redsa"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("sly_adnanced_redstone:redsa"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ModTrigger().setRegistryName("redsa"));
	}

	public static class ModTrigger extends Item {
		public ModTrigger() {
			super(new Item.Properties().group(MCreatorRWE.tab).maxDamage(64));
		}
		...
		public ActionResultType onItemUse(ItemUseContext context) {
			...
			if (!entity.canPlayerEdit(pos, context.getFace(), itemstack)) {
				...
				int z = pos.getZ();
				{
					...
					MCreatorSadaPortalTriggerUsed.executeProcedure($_dependencies);
				}
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(("block.note_block.bit"))),
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == MCreatorUncraftable.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == MCreatorUncraftable.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == MCreatorUncraftable.block.getDefaultState().getBlock())) {
								break label56;
							}
				...
				for (int j7 = -1; j7 <= 1; ++j7) {
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos, flag ? MCreatorUncraftable.block.getDefaultState().getBlock()
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, MCreatorUncraftable.block.getDefaultState().getBlock().getDefaultState(),
								3);
					}
				}
			}
	...
	public static class CustomDimension extends Dimension {
		...
		public Vec3d getFogColor(float cangle, float ticks) {
			return new Vec3d(0.874509803922, 0, 0);
		}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return MCreatorRedston.block.getDefaultState();
				}
		...
		public BiomeProviderCustom(World world) {
			...
			for (Biome biome : this.biomes) {
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(MCreatorRedston.block.getDefaultState().getBlock(), biome.getSurfaceBuilder().getConfig()
								.getTop().getBlock(), biome.getSurfaceBuilder().getConfig().getUnder().getBlock());
					}
