...
package net.mcreator.sly_adnanced_redstone.particle;
...
public class RedParticle extends SlyAdnancedRedstoneModElements.ModElement {
	...
	public void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("red"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		public void tick() {
			super.tick();
			if (!this.isExpired) {
				this.setSprite(this.spriteSet.get((this.age / 7) % 1 + 1, 1));
			}
		}
	}
